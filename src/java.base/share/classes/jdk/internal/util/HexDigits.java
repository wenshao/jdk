...
public final class HexDigits {
    ...
    public static int getCharsUTF16(long value, int index, byte[] buffer) {
        while ((value & ~0xFF) != 0) {
            ...
            JLA.putUTF16Char(buffer, --index, pair >> 8);
            JLA.putUTF16Char(buffer, --index, pair & 0xFF);
            value >>>= 8;
        }

        int digits = DIGITS[(int) (value & 0xFF)];
        JLA.putUTF16Char(buffer, --index, (byte) (digits >> 8));

        if (0xF < value) {
            JLA.putUTF16Char(buffer, --index, (byte) (digits & 0xFF));
        }
